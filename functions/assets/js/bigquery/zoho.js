...
async function getUsersBQ(req, res) {
    ...
    try {
        ...
        if (!results.length) {
            return res.status(200).json({ success: true, message: "No hay nuevos registros para enviar." });
        }
...
async function ensureCursorTableExists() {
    const [datasetId, tableId] = CURSOR_TABLE.split('.').slice(1); // ["Users", "cursor_zoho_users"]
